# BranchBase contributor instructions

- Use Bun for dependencies, scripts, tests, and local development.
- Keep repository-specific behavior in `.branchbase.json`; do not add project-name or folder-name conventions to the product.
...
- BranchBase is macOS-first. Platform-dependent behavior belongs behind the host or process-inspection seams.
...
BranchBase uses a single-context domain model. See `docs/agents/domain.md`.
